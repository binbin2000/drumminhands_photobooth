...
total_pics = config.frame_count  # number of pics to be taken
...
offset_x = 0  # how far off to l eft corner to display photos
    ...
    if config.capture_count_pics:
        try:  # take the photos
            for i in range(1, total_pics+1):
                ...
                # take_pic_command = "gphoto2 --capture-image-and-download --filename " + filename + " --force-overwrite -F " + str(config.frame_count) + " -I " + str(config.frame_interval)
                take_pic_command = "gphoto2 --capture-image-and-download --filename " + filename + " --force-overwrite"
                # executes command below
                sub.Popen(take_pic_command, stdout=sub.PIPE, stderr=sub.PIPE, shell=True)
                ...
                capture_delay = config.frame_interval + config.camera_delay
                time.sleep(capture_delay)   # pause after shots
                clear_screen()
                if i == total_pics + 1:
                    break
